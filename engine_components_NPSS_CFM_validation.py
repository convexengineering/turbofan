...
##                    TCS([etaB * f*hf >= (1)*ht4-(1)*ht3]),
##                    TCS([etaB * f*hf + ht3 >= ht4]),
##                    SignomialEquality(etaB *f * hf, ht4-ht3),
##                    f <= .1,
    ...
    def __init__(self, **kwargs):
        ...
        with SignomialsEnabled():
            constraints = [
                ...
                TCS([W_engine >= (mCore*.0984)*(1684.5+17.7*(pif*pilc*pihc)/30+1662.2*(alpha/5)**1.2)*units('m/s')]),
                ]
...
##                            + 1.22e-50 * (N1)**-2.84e+03 * (mtildlc)**598)),
                ...
                constraints=[
                    pilc*(26/pilcD) == (1.35 * (N1)**0.383)**10,
                    pilc*(26/pilcD) <= 1.02*(1.38 * (mtildlc)**0.122)**10,
                    pilc*(26/pilcD) >= .98*(1.38 * (mtildlc)**0.122)**10,
                    ]
...
##                            + 1.22e-50 * (N2)**-2.84e+03 * (mtildhc)**598)),
                ...
                constraints=[
                    ...
                    pihc*(26/pihcD) >= .98*(1.38 * (mtildhc)**0.122)**10,
                    pihc*(26/pihcD) <= 1.02*(1.38 * (mtildhc)**0.122)**10,
                    ]
...
##                            + 0.174 * (Nf)**-0.124 * (mtildf)**0.0104)),
                ...
                constraints = [
                    ...
                    pif*(1.7/piFanD) >= .98*(1.04 * ((mtildf)**0.022))**10,
                    pif*(1.7/piFanD) <= 1.02*(1.04 * ((mtildf)**0.022))**10,

##                    pif*(26/piFanD) == (1.35 * (Nf)**0.383)**10,
##                    pif*(26/piFanD) >= .95*(1.38 * (mtildf)**0.122)**10,
##                    pif*(26/piFanD) <= 1.05*(1.38 * (mtildf)**0.122)**10,
                ]
        constraints.extend([
            #define mbar
