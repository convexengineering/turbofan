...
from gpkit.nomials import SignomialEqualityLin, SignomialEqualityLinTrust
...
##                f*hf + ht3 >= ht4,
               ...
               SignomialEqualityLin(fp1+0,f+1),
            ...
            if cooling == True:
                constraints.extend([
                    ...
                    SignomialEqualityLin(fp1*u41-ac*uc, (u4a*(1-ac)+f*u4a)),          
    ...
    def __init__(self, **kwargs):
        ...
        with SignomialsEnabled():
            constraints = [
                ...
                #SIGNOMIAL   
                SignomialEqualityLin(Mtakeoff*etaHPshaft*(1+f)*(ht41-ht45),ht3 - ht25),    #B.161

                #LPT shaft power balance
                #SIGNOMIAL  
                SignomialEqualityLin(Mtakeoff*etaLPshaft*(1+f)*(ht49 - ht45),-((ht25-ht18)+alpha*(ht21 - ht2))),    #B.165
    ...
    def __init__(self, **kwargs):
        ...
        with SignomialsEnabled():
            constraints = [
                ...
                h8 == Cpfanex * T8,
##                TCS([u8**2 + 2*h8 <= 2*ht8]),
                SignomialEqualityLin(u8**2 + 2*h8, 2*ht8+0),
                (P8/Pt8)**(.2857) == T8/Tt8,
                ht8 == Cpfanex * Tt8,
...
##                        + 1.22e-50 * (N1)**-2.84e+03 * (mtildlc)**598)]),
               SignomialEqualityLinTrust((10*pilc)**(-.163) , ( 5.28e-10 * (N1)**-88.8 * (mtildlc)**2.05
                        + 0.0115 * (N1)**-16.5 * (mtildlc)**4.89
                        + 0.575 * (N1)**-0.491 * (mtildlc)**-0.0789
                        + 7.99e-15 * (N1)**2.07e+04 * (mtildlc)**592
                        + 1.22e-50 * (N1)**-2.84e+03 * (mtildlc)**598)),
...
##                        + 1.22e-50 * (N2)**-2.84e+03 * (mtildhc)**598)]),
               ...
               SignomialEqualityLinTrust((3*pihc)**(-.163) , ( 5.28e-10 * (N2)**-88.8 * (mtildhc)**2.05
                        + 0.0115 * (N2)**-16.5 * (mtildhc)**4.89
                        + 0.575 * (N2)**-0.491 * (mtildhc)**-0.0789
                        + 7.99e-15 * (N2)**2.07e+04 * (mtildhc)**592
                        + 1.22e-50 * (N2)**-2.84e+03 * (mtildhc)**598)),
...
class FanMap(Model):
    ...
    def __init__(self, **kwargs):
        ...
        with SignomialsEnabled():
            constraints = [
               SignomialEqualityLinTrust(pif**(-.187), (0.179 * (Nf)**-0.14 * (mtildf)**0.0288
