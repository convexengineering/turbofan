...
class Turbine_losses(Model):
    ...
    def __init__(self, **kwargs):
        ...
        losses = Variable('losses', '-', 'percent mass flow loss for deice, pressurization, etc.')

        with SignomialsEnabled():
            constraints = [
                #HPT shafter power balance
                #SIGNOMIAL   
                SignomialEquality(losses*(1+f)*(ht41-ht45),ht3 - ht25),    #B.161

                #LPT shaft power balance
                #SIGNOMIAL  
                SignomialEquality(losses*(1+f)*(ht49 - ht45),-((ht25-ht18)+alpha*(ht21 - ht2))),    #B.165
...
class ExhaustAndThrust_losses(Model):
    """
...
##                TCS([F <= F6 + F8]),
                ...
                F + alphap1*mCore*u0 <= losses*mCore*u6+mCore*alpha*u8, 
...
class OnDesignSizing_losses(Model):
    """
    ...
    def __init__(self, m6opt, m8opt, cooling, tstages, **kwargs):
        ...
        with SignomialsEnabled():
            constraints = [
                ...
                #mass flow sizing
                losses*mCore == Fd/(Fsp*a0*(alphap1)),  #B.194
                ...
                mhtD == fp1*losses*mCore*((Tt41/Tref)**.5)/(Pt41/Pref), #B.225
                mltD == fp1*losses*mCore*((Tt45/Tref)**.5)/(Pt45/Pref), #B.226
